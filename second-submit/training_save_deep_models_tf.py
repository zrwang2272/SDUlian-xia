...
import argparse
from help_code_demo_tf_2d import ECG_DataSET, ToTensor, create_dataset, FB, Sensitivity  # 您可能需要调整这部分，以确保数据加载和转换与 TensorFlow 兼容
...
def main():
    ...
    writeFile.write('beiyes best 1:3\n')
    ...
    for epoch in range(EPOCH):
        ...
        writeFile.write('Test Acc: %.5f Test Loss: %.5f\n' % (correct / total, running_loss_test / i))
        ...
        writeFile.write(f"G Score: {proportion_above_threshold:.5f}\n")
    ...
    # Save model
    net.save('./saved_models/ECG_net_tf_{}.h5'.format(t))
