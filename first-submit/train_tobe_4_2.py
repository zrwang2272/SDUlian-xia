...
from tensorflow.keras.layers import Dense, Conv2D, Dropout, MaxPooling2D, Flatten,Conv1D
...
def get_model(fil=9, k_size=7, fil_2=8,k_size_2=15,fil_3=13, k_size_3=3,fil_4=14, k_size_4=13):
    ...
    model = Sequential([
        ...
        # Conv1D(filters=int(fil_2), kernel_size=int(k_size_2), strides=2, padding='valid', activation='relu',  name="conv1d_2"),
        # Conv1D(filters=int(fil_3), kernel_size=int(k_size_3), strides=2, padding='valid', activation='relu',  name="conv1d_3"),
        # Conv1D(filters=int(fil_4), kernel_size=int(k_size_4), strides=2, padding='valid', activation='relu',  name="conv1d_4"),
        # layers.MaxPooling2D(pool_size=(2,1)),
        Flatten(name="flatten"),
        Dense(10, activation='relu', name="dense_1"),
        Dense(2, activation=None, name="dense_2")
    ])
    return model
...
def fit_with(fil,k_size, fil_2,k_size_2, fil_3, k_size_3, fil_4, k_size_4, verbose, lr):
    ...
    input_shape = (1250, 1)
    # 使用指定的超参数创建模型
    model = get_model(int(fil),int(k_size),int(fil_2),int(k_size_2),int(fil_3), int(k_size_3),int(fil_4), int(k_size_4))
...
def main():
    ...
    writeFile.write('1D 4 tobe with original device\n')
    ...
    pbounds = {'fil':(5,15),'k_size': (5,18), 'fil_2':(5,15),'k_size_2': (10,20), 
                'fil_3':(7,18),'k_size_3': (3,10), 'fil_4':(7,23),'k_size_4': (7,19)}
